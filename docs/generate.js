...
/** 
 * Safely escape a string for use as a JS string literal inside an HTML onclick attribute.
 */
...
function generateFunctionCard(moduleName, fn) {
  ...
  const postUrl = `${CONVEX_URL}/api/run/${moduleName}/${fn.name}`;
  const urlDisplay = !fn.type.startsWith('internal')
    ? `<div class="code-label" style="margin-top:14px;">POST URL</div>
       <div class="url-bar">
         <code>${postUrl}</code>
         <button class="copy-btn" onclick="copyToClipboard(this, '${postUrl}')" title="Copy URL">${COPY_SVG}</button>
       </div>`
    : '';
  ...
  const jsonBlock = jsonExample
    ? `<div class="code-label" style="margin-top:14px;">Body / Arguments</div>
        <div class="code-wrap">
          <pre><code class="language-json">${escapeHtml(jsonExample)}</code></pre>
          <button class="copy-btn" onclick="copyToClipboard(this, '${escapeForOnclick(jsonExample)}')" title="Copy JSON">${COPY_SVG}</button>
        </div>`
    : '';
  ...
  return `
    <div class="card" id="${anchorId}">
      ...
      <div class="card-body">
        ...
        <div class="code-wrap">
          <pre><code class="language-typescript">${escapeHtml(exampleCall)}</code></pre>
          <button class="copy-btn" onclick="copyToClipboard(this, '${escapeForOnclick(exampleCall)}')" title="Copy Example">${COPY_SVG}</button>
        </div>
      </div>
    </div>`;
}
...
function generateHttpCard(route) {
  ...
  const fullUrl = `${CONVEX_SITE_URL}${route.path}`;
  return `
    <div class="card" id="${anchorId}">
      <div class="card-header">
      ...
      <div class="card-body">
        <div class="code-label">Endpoint URL</div>
        <div class="url-bar">
          <code>${fullUrl}</code>
          <button class="copy-btn" onclick="copyToClipboard(this, '${fullUrl}')" title="Copy URL">${COPY_SVG}</button>
        </div>
        <p class="fn-desc" style="margin-top:14px;">See <a href="swagger.html" style="color:var(--accent2)">Swagger UI</a> for full documentation and interactive testing.</p>
      </div>
    </div>`;
}
...
function generateSwaggerJson(routes) {
  ...
  return {
    ...
    servers: [{ url: CONVEX_SITE_URL, description: 'Production' }],
    paths,
  };
}
...
function buildIndexHtml(modules, httpRoutes) {
  ...
  params: {
    ...
    password: ""
          ...
          <button class="copy-btn" onclick="copyToClipboard(this, '${escapeForOnclick(JSON.stringify({
  provider: "password",
  params: {
    flow: "signIn",
    email: "",
...
}, null, 2))}')" title="Copy JSON">${COPY_SVG}</button>
        </div>
        <div class="code-label" style="margin-top:14px;">Example</div>
...
await signOut();</code></pre></div>
  ...
  <div class="section" id="httpEndpoints">
    ...
    <div class="section-desc">
      Standard HTTP routes at <code>${CONVEX_SITE_URL}</code>.
...
<head>
  ...
  <style>
    *, *::before, *::after { box-sizing: border-box; margin: 0; padding: 0; }
    ...
    .url-bar { background: #f0f0f8; border: 1px solid var(--border); border-radius: 6px; padding: 10px 14px; margin-bottom: 16px; font-family: var(--mono); font-size: 12px; color: var(--accent2); word-break: break-all; position: relative; padding-right: 40px; }
    .code-wrap { border-radius: 8px; overflow: hidden; font-size: 12.5px; line-height: 1.55; position: relative; }
    .copy-btn { position: absolute; top: 8px; right: 8px; background: rgba(0,0,0,0.05); border: 1px solid rgba(0,0,0,0.1); border-radius: 4px; padding: 4px; cursor: pointer; display: flex; align-items: center; justify-content: center; transition: all 0.2s; z-index: 10; }
    .copy-btn:hover { background: rgba(0,0,0,0.1); border-color: rgba(0,0,0,0.2); }
    .copy-btn svg { width: 14px; height: 14px; fill: var(--text-muted); }
    .code-wrap .copy-btn { background: rgba(255,255,255,0.1); border-color: rgba(255,255,255,0.1); }
    .code-wrap .copy-btn:hover { background: rgba(255,255,255,0.2); }
    .code-wrap .copy-btn svg { fill: #fff; opacity: 0.7; }
...
function main() {
  ...
  try {
    ...
    swagger.servers = [{ url: CONVEX_SITE_URL, description: 'Production' }];
